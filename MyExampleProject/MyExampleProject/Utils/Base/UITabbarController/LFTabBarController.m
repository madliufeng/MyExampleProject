...
{
    [self setupChildVc:[[LFHomeViewController alloc] init] title:@"首页" image:@"lrhicon_home" selectedImage:@"lrhicon_home_highlight"];
    
    [self setupChildVc:[[LFListViewController alloc] init] title:@"课堂" image:@"icon_schedule" selectedImage:@"icon_schedule_hightlight"];
    
    [self setupChildVc:[[LFDiscoverViewController alloc] init] title:@"发现" image:@"lrhicon_found" selectedImage:@"lrhicon_found_hightlight"];
    
    [self setupChildVc:[[LFMineViewController alloc] init] title:@"我" image:@"icon_user" selectedImage:@"icon_user_highlight"];}
...
- (BOOL)tabBarController:(UITabBarController*)tabBarController shouldSelectViewController:(UINavigationController*)viewController {
    /// 特殊处理 - 是否需要登录
    BOOL isBaiDuService = [viewController.topViewController isKindOfClass:[LFHomeViewController class]];
    if (isBaiDuService) {
        NSLog(@"你点击了TabBar第二个");
    }
    return YES;
