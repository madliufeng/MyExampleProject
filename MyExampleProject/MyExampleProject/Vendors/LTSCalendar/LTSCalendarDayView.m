...
//    circleView.color = [UIColor clearColor];
    {
        ...
        //DFE9E4
        _circleView.backgroundColor = [UIColor colorWithHexString:@"08ce5b"];
...
//    circleView.layer.borderWidth = 1;
    _circleView.frame = CGRectMake(0, 0, self.bounds.size.width-2, self.bounds.size.height-2);
...
#warning TODO:判断农历是否显示
    ...
    if (self.calendarManager.calendarAppearance.isShowLunarCalender) {
        ...
        textLabel.center =  CGPointMake(_circleView.center.x, _circleView.center.y- CGRectGetHeight(textLabel.frame)/2);
        lunarTextLabel.center = CGPointMake(_circleView.center.x, _circleView.center.y + CGRectGetHeight(lunarTextLabel.frame)/2+2);
    }
    else{
        textLabel.center = _circleView.center;
        
        lunarTextLabel.hidden = YES;
    }
   
    
    dotView.frame = CGRectMake(0, CGRectGetMaxY(_circleView.frame)+1, sizeDot, sizeDot);
...
- (void)setSelected:(BOOL)selected animated:(BOOL)animated{
    ...
    if(selected){
        if(!self.isOtherMonth){
//            _circleView.color = [self.calendarManager.calendarAppearance dayCircleColorSelected];
...
//        _circleView.color = [UIColor clearColor];
        if ([self isToday]){
           ...
           _circleView.layer.borderColor = [self.calendarManager.calendarAppearance dayBorderColorToday].CGColor;
    ...
    if(animated){
        [UIView animateWithDuration:.1 animations:^{
            _circleView.layer.opacity = opacity;
            _circleView.transform = tr;
        }];
    }
    else{
        _circleView.layer.opacity = opacity;
